...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    # def help_func(index, ret, step):
    #     if index == n:
    #         return ret
    #     elif num_eights(index) > 0 or index % 8 == 0:
    #         return help_func(index + 1, ret - step, -step)
    #     else:
    #         return help_func(index + 1, ret + step, step)
    # return help_func(1, 1, 1)
    # def help_next(idx, step):
    #     if idx % 8 == 0 or num_eights(idx) > 0:
    #         return -step
    #     return step
    #
    # def help_pingpong(ret, i, step):
    #     if i == n:
    #         return ret
    #     return help_pingpong(ret + step, i + 1, help_next(i + 1, step))
    # return help_pingpong(1, 1, 1)
    def help_func(index):
        if index <= 8:
            return index
        return step(index) + help_func(index - 1)

    def step(idx):
        if idx < 8:
            return 1
        elif (idx - 1) % 8 == 0 or num_eights(idx - 1) > 0:
            return -1 * step(idx - 1)
        return step(idx-1)
